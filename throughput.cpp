    ...
    ui(new Ui::ThroughPut)
{
    ...
    InputManagement();//以读入文件，本程序没有使用文件。
    size_chl2_3=sizeof(sockaddr_in);//socket初始化
    ...
    addrSrv_chl1_3.sin_port=htons(7024);//server : receive port number  127.0.0.1:7024
    ...
    id1 = startTimer(100);//启动时间刷新函数：『105』
...
void ThroughPut::resizeGL(int w, int h)
{
    ...
    //glOrtho (-1.5 * ( GLfloat ) w / ( GLfloat ) h, 2.3* ( GLfloat ) w / ( GLfloat ) h, -2, 2, -15.0, 15.0);
     ...
     glOrtho (-2.4* ( GLfloat ) w / ( GLfloat ) h, 2.2* ( GLfloat ) w / ( GLfloat ) h, -9, 1.8, -10.0, 15.0);
...
void ThroughPut::Bar_ber(){
    ...
    glColor4f(0.5,0,1,0.8);//purple
    glVertex2f(-3,2); //left up
    glVertex2f(-3,-1);// left,down
    glVertex2f(3,-1);//right down
    glVertex2f(3,2);//right up  [-3 2.8 2 -1]
    glEnd();
//second frame
    glBegin(GL_LINE_LOOP);
    glColor4f(0.5,0,1,0.8);//purple
    glVertex2f(-3,-1.2); //left up
    glVertex2f(-3,-4.2);// left,down
    glVertex2f(3,-4.2);//right down
    glVertex2f(3,-1.2);//right up  [-3 2.8 2 -1]
    glEnd();
    //third frame
    glBegin(GL_LINE_LOOP);
    glColor4f(0.5,0,1,0.8);//purple
    glVertex2f(-3,-4.4); //left up
    glVertex2f(-3,-7.4);// left,down
    glVertex2f(3,-7.4);//right down
    glVertex2f(3,-4.4);//right up  [-3 2.8 2 -1]
    glEnd();
   ...
   comp_err[4] = comp_err[8];
    ...
    for( int i = 0 ; i < 8 ; i++ ){//8 histograms
    glBegin(GL_QUADS);
    glColor4f(1,i/9.0,0,0.5);
    glVertex2f(-1.4+pos,-1);//x,y
    glVertex2f(-1.4+pos,height2[i]);
    glVertex2f(-2+pos,height2[i]);
    glVertex2f(-2+pos,-1);
     glEnd();
     pos  = (i*0.7);
    }
     ...
     pos = -0.7;
    for( int i = 0 ; i < 4 ; i++ ){//4 histograms
    glBegin(GL_QUADS);
    glColor4f(1,i/9.0,0,0.5);
    glVertex2f(-1+pos,-4.2);//x,y
    glVertex2f(-1+pos,height2[i]-3);
    glVertex2f(-2+pos,height2[i]-3);
    glVertex2f(-2+pos,-4.2);
     glEnd();
     pos += (1)*1.4;
    }
    ...
    pos = -0.7;
   for( int i = 0 ; i < 2 ; i++ ){//2 histograms
   glBegin(GL_QUADS);
   glColor4f(1,i/9.0,0,0.5);
   glVertex2f(pos,-7.4);//x,y
   glVertex2f(pos,height2[i]-6);
   glVertex2f(-2+pos,height2[i]-6);
   glVertex2f(-2+pos,-7.4);
    glEnd();
    pos += (1)*2.8;
   }
    /*error == 1 */
    ...
    glColor3f(1,0,0);
    renderText(-3.5,2.2,1,"");//danwei  显示文本信息 x,y,z,string   （x坐标，y坐标，z坐标，字符串）
    glLineWidth(0.5);
    glBegin(GL_LINE_STRIP);
    glColor4f(0,0,1,0.8);
    glVertex2f(-3,1);//左顶点坐标
    glVertex2f(3,1);//x, y右顶点坐标
    glEnd();

    glColor3f(1,0,0);
    renderText(-3.5,1,1,"100%");//显示文本信息 x,y,z,string   （x坐标，y坐标，z坐标，字符串）
    glLineWidth(0.5);
    glBegin(GL_LINE_STRIP);
    glColor4f(0,0,1,0.8);
    glVertex2f(-3,0.5);//left point
    glVertex2f(3,0.5);//x,y  right point
    glEnd();
    glColor3f(1,0,0);
    renderText(-3.5,0.5,1,"75%");//显示文本信息 x,y,z,string   （x坐标，y坐标，z坐标，字符串）
    glLineWidth(0.5);
    glBegin(GL_LINE_STRIP);
    glColor4f(0,0,1,0.8);
    glVertex2f(-3,0);
    ...
    glEnd();
    glColor3f(1,0,0);
    renderText(-3.5,0,1,"50%");//显示文本信息 x,y,z,string   （x坐标，y坐标，z坐标，字符串）
    glLineWidth(0.5);
    glBegin(GL_LINE_STRIP);
    glColor4f(0,0,1,0.8);
    glVertex2f(-3,-0.5);
    glVertex2f(3,-0.5);//x,y
    glEnd();

    glColor3f(1,0,0);
    renderText(-3.5,-0.5,1,"25%");
    glLineWidth(0.5);
    glBegin(GL_LINE_STRIP);
    glColor4f(0,0,1,0.8);
    glVertex2f(-0.5,0.5);
    ...
    char title[9][50];//显示一组下标文本
    sprintf(title[0],"16QAM");//向字符串中填充内容
    ...
    if(1){
        ...
        for( int i = 0 ; i < 2 ; i ++ ){
            ...
            if( i == 0 ){
                sprintf(db_2,"BER:%.3lf",1+(height2[i]));//向字符串中填充内容
            }else{
                sprintf(db_2,"FER:%.3lf",1+(height2[i]));
            }
           renderText(-2.5+ i *1.6,-1.3,1.1,db_2);//显示文本信息 x,y,z,string   （x坐标，y坐标，z坐标，字符串）
            //comp_err[i] = 0;
        }
...
void ThroughPut::timerEvent(QTimerEvent *event){
    ...
    recvfrom(sockser_chl1_3,&buff,14404*3+10,0,(struct sockaddr *)&addrrcv_chl1_3,(socklen_t*)&size_chl2_3);//port :7005.3  socket接受函数
   ...
   //qDebug() << buff << endl;
    ...
    int position = 12; // avoid the header a0aa 3c20 cccc  初始偏移量
